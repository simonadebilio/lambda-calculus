...
\begin{center}
\AxiomC{$S : (\zeta \rightarrow \zeta) \rightarrow \epsilon $} 

\AxiomC{$I: \zeta \rightarrow \zeta$}
\BIC{$\mathbf{SI} : (\delta \rightarrow \delta) \rightarrow \epsilon $}

\AxiomC{$ I: \delta \rightarrow \delta$}
\BIC{$\vdash \mathbf{SII} : \epsilon$}
\DP
\end{center}
...
\subsubsection*{iii) $\lambda xy. y(\lambda z.(yz))$}
